...
{% include image.html
   ...
   caption= "Fig. 1: Block diagram of the proposed framework."
%}
...
leaving homeowners unaware of potential threats until it is too late. Motivated by the need for proactive security measures, 
this article introduces a novel approach that leverages Large Language Models (LLMs) to serve as virtual home security guards. 
</div>
...
The proposed model integrates LLMs into the existing home security infrastructure to create a proactive notification system. The block diagram is shown in <a href="#fig1">Fig. 1</a>. The framework operates as follows:

**Video Understanding**: Home surveillance cameras capture video footage of the surrounding environment, and video segments are then extracted by an action detection model, e.g., [[1][action_detection]]. 
These video segments, together with a prompt, are fed into a Multi-modal Large Language Model (MLLM), such as [[2][ask_anything]], which analyzes the scene to understand the video content — i.e., abnormal or unexpected human activities — and answers the user's questions based on the given prompt.
The prompt fed into the LLM is as follows:
{% highlight Python %}
...
Users receive instant notifications on their devices, alerting them to potential security threats.
...
{% include image.html
   ...
   caption= "Fig. 2: A case demonstrating an unfamiliar individual approaching the doorstep."
%}
...
Note that the "!init" command is used for offline demonstration purposes; in the real-time setting, the LLM receives visual information directly from the video stream.
